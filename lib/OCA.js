...
var f = function(w, OCA) {
 ...
 var event_signature = new SP.signature(OCA.OcaEvent);
 ...
 function do_unsubscribe(event, method) {
 }
 function subscribe_cb(key, o) {
     var S = this.subscriptions[key];
     if (!S) {
        OCA.warn("Subscription lost.\n");
        return;
     }
     var a = S.callbacks;
 ...
 RemoteDevice.prototype = {
     ...
     add_subscription: function(event, callback) {
        ...
        var cb = subscribe_cb.bind(this, key);
     ...
     remove_subscription: function(event, callback) {
        var key = event_signature.encode(event)
        key = String.fromCharCode.apply(String, new Uint8Array(key));
        var S = this.subscriptions[key];
